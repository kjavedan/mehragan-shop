...
export default function CategoryPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    const items: Array<{ name: string; subcategory: string; category: string }> = []
  ...
  const getItemId = (item: { name: string; subcategory: string; category: string }) =>
    `${item.category}-${item.subcategory}-${item.name}`

  const isItemInCart = (item: { name: string; subcategory: string; category: string }) => {
    const itemId = getItemId(item)
    return items.some((cartItem) => cartItem.id === itemId)
  }

  const getCartItemQuantity = (item: { name: string; subcategory: string; category: string }) => {
    const itemId = getItemId(item)
    const cartItem = items.find((cartItem) => cartItem.id === itemId)
    return cartItem?.quantity || 1
  }

  const handleItemSelect = (item: { name: string; subcategory: string; category: string }, checked: boolean) => {
    const itemId = getItemId(item)
  ...
  const handleQuantityChange = (item: { name: string; subcategory: string; category: string }, quantity: number) => {
    const itemId = getItemId(item)
  ...
  return (
    <div className="max-w-7xl mx-auto relative pb-20">
      ...
      <div className={`sticky bg-white w-full top-16 z-40 ${isSearchSticky ? 'pt-4': ''}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 my-6 px-4 sm:px-6 lg:px-8">
        {filteredItems.map((item) => {
          ...
          const quantity = isSelected ? getCartItemQuantity(item) : quantities[item.name] || 1
              ...
              <CardContent className="p-3">
                <div className="flex items-center space-x-3 space-x-reverse">
                  ...
                  <div className="flex-shrink-0">
                    <QuantityInput value={quantity} onChange={(value) => handleQuantityChange(item, value)} min={1} />
                  </div>
                </div>
              </CardContent>
            </Card>
